  ...
  return (
    <aside className="rounded bg-white shadow-sm">
      ...
      <div className="divide-y divide-gray-100">
        {bookmakers.map((bm) => (
          <div key={bm.rank} className="flex items-center gap-3 px-3 py-2.5">
            <div
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className="text-xs font-medium text-gray-500">
                {bm.url === "#" ? "License and offer pending review" : bm.bonus}
              </span>
            </div>
            {bm.url === "#" ? (
              <span className="shrink-0 rounded bg-gray-100 px-2 py-1 text-[9px] font-bold uppercase text-gray-500">Pending</span>
            ) : (
              <a href={bm.url} target="_blank" rel="noopener noreferrer sponsored" className="btn-green shrink-0 px-2 py-1 text-[10px]">VIEW</a>
            )}
          </div>
        ))}
      </div>
